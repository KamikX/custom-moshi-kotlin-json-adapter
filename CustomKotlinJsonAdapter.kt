...
/**
 *  Custom adapter based on this branch :
 *  https://github.com/square/moshi/blob/eric.2019-01-15.multiple-transient-constructor-parameters/kotlin/reflect/src/main/java/com/squareup/moshi/kotlin/reflect/KotlinJsonAdapter.kt
 *  + own exclude from json annotation @ExcludeFromJson
 *  + disabled exclude from json with @Transient annotation
 */
 ...
 */
internal class CustomKotlinJsonAdapter<T>(
    ...
    override fun toString() = "CustomKotlinJsonAdapter(${constructor.returnType})"
...
class CustomKotlinJsonAdapterFactory : JsonAdapter.Factory {
    override fun create(type: Type, annotations: MutableSet<out Annotation>, moshi: Moshi)
        ...
        val bindingsByName = LinkedHashMap<String, CustomKotlinJsonAdapter.Binding<Any, Any?>>()
        ...
        for (property in rawTypeKotlin.memberProperties) {
            ...
            var excludeAnnotation = property.findAnnotation<ExcludeFromJson>()
            ...
            if (parameter != null) {
                ...
                if (excludeAnnotation == null) {
                    excludeAnnotation = parameter.findAnnotation()
                }
            }

            // Skip if is exclude annotation detected
            if(excludeAnnotation != null) {
                continue
            }
            ...
            bindingsByName[property.name] =
                CustomKotlinJsonAdapter.Binding(
                    name, adapter,
                    property as KProperty1<Any, Any?>, parameter, parameter?.index ?: -1
                )
        }

        val bindings = ArrayList<CustomKotlinJsonAdapter.Binding<Any, Any?>?>()
        ...
        return CustomKotlinJsonAdapter(
            constructor,
            bindings,
